...
// Compose the final gpt-image-1 prompt from the geo spec. The STYLE is fixed here (not left to
// the LLM) so every geo image looks like the same clean broadcast-map reference set.
export function frameGeoPrompt(spec = {}) {
  const accent = PALETTE_DESC[spec.palette] || PALETTE_DESC.red;
  const base = `A clean, modern broadcast-news explainer MAP graphic in a flat vector cartography style. Show ${spec.region}. Light, slightly desaturated landmasses in pale grey-green, soft pale-blue water, thin neutral country borders, plenty of white/light negative space. Minimal, editorial, calm — like a newspaper or TV locator map.`;

  let focus;
  if (spec.mapType === "radius") {
    const rings = spec.rings.length ? spec.rings.join("; ") : "Effect radius; Outer reach";
    focus = `Overlay concentric translucent ${accent} rings centered over the area to suggest an effect-radius / reach diagram, soft glowing fills fading outward. Small dark rounded pill labels with white text read exactly: ${rings}. Keep the labels short, legible and correctly spelled.`;
  } else {
    const lbl = spec.hotspotLabel || "Hotspot";
    focus = `Draw a bold ${accent} circle outlining ${spec.hotspot || "the key chokepoint"}. A small dark crimson rounded-rectangle label box with clean white sans-serif text reading exactly "${lbl}" sits nearby, connected to the circle by a thin straight line. ${spec.countryLabels.length ? `Print the place names ${spec.countryLabels.join(", ")} faintly on the map in light grey uppercase.` : ""} Keep all text short, legible and correctly spelled.`;
  }

  const alert = spec.threatLabel ? ` A small neutral tag reading "${spec.threatLabel}" may appear discreetly.` : "";
