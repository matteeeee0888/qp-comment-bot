...
async function anthropicJSON(system, user) {
  ...
  const body = JSON.stringify({ model, max_tokens: 1024, system, messages: [{ role: "user", content: `${user}\n\nRespond with ONLY the JSON object, no prose, no code fences.` }] });
...
export async function genJSON(system, user) {
  if (ENGINE === "anthropic") return anthropicJSON(system, user);
  if (ENGINE === "openai") return openaiJSON(system, user);
