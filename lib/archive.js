...
export async function archiveNews(rows) {
  ...
  try {
    ...
    // The News branch replies "ok <n>" (with a count). A bare "ok" means the OLD script ran
    // (request fell through to the comments path) → the new version isn't deployed yet.
    const body = (await r.text()).slice(0, 300).trim();
    return { ok: r.ok && /^ok\s+\d+/i.test(body), status: r.status, body, count: rows.length };
  } catch (e) {
    return { ok: false, error: String(e.message || e) };
  }
}
