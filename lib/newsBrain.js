...
// Where buyers come from (funnel quiz) — bias scoring toward stories hitting buyer-heavy states.
const _repoRoot = fileURLToPath(new URL("../", import.meta.url));
const BUYER_STATES = JSON.parse(await readFile(path.join(_repoRoot, "data/buyer-states.json"), "utf8")).states;
const TOP_BUYER = BUYER_STATES.slice(0, 16).map((s) => s.state).join(", ");
...
export const BRANDS = {
  ...
  terrastryke: "TerraStrike — keychain car-escape tool (spring striker for tempered side windows + seatbelt cutter). Triggers: vehicle escape, cars in floodwater, being trapped in a car, driving-season & roadside safety.",
...
AUDIENCE: older US adults who keep cable/satellite news (Fox, CNN, MSNBC/MS NOW, Al Jazeera) on all day — they trust and respond to the plain-spoken, urgent framing of TV news. Favor stories already dominating those channels, and echo that familiar framing in the hook — but stay calm, factual and compliant (readiness, never doom or fear-mongering).
...
- b Audience Breadth — how many of OUR buyers it touches. Our buyers concentrate in these states (most first): ${TOP_BUYER}. A story hitting those states scores HIGHER on b; one only in a tiny / low-buyer state scores lower
