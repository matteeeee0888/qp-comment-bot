...
// List object names directly under a prefix (one folder level), e.g. listFolder("news/") → date
// folders, listFolder("news/2026-06-16/") → the PNGs in that day. Returns [] on any failure.
export async function listFolder(prefix, { limit = 1000 } = {}) {
  const c = await load();
  if (!c.url || !c.key || !c.bucket) return [];
  try {
    ...
    if (!r.ok) return [];
    return (await r.json()).filter((e) => e && e.name).map((e) => e.name);
  } catch { return []; }
}
