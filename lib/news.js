...
// Fetch a direct publisher RSS feed (Fox, NBC, Al Jazeera, ...). The audience skews older and
// TV-news-driven, so newsjacking the exact stories on their home channels lands harder. Direct
// feeds omit the <source> tag Google News injects, so we stamp it from the caller.
export async function fetchRss(url, { source = "", label = "", timeoutMs = 15000 } = {}) {
  const ctrl = new AbortController();
  const t = setTimeout(() => ctrl.abort(), timeoutMs);
  try {
    ...
    if (!r.ok) return [];
    const xml = await r.text();
    return parseItems(xml).map((it) => ({ ...it, source: it.source || source, query: label || source }));
  } catch {
    return []; // a single dead feed never sinks the run
  } finally {
    clearTimeout(t);
